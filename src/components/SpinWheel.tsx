...
const SpinnerWheel: React.FC<ProfileProps> = ({ userData }) => {
  ...
  const handleSpinStop = async () => {
    ...
    const updateData: any = { totalSpins: spinsLeft };
  ...
  const handlePayment = async (amount: number) => {
    ...
    try {
      ...
      console.log(dep);
      await updateUserProfile({ totalSpins: userDeets.totalSpins + 1 });
      toast({ title: "Deposit successful", status: "success", duration: 3000, isClosable: true });
      setSpinsLeft((prev) => prev + 1);
      setTotalWinnings(0);
    } catch (error) {
  ...
  return (
    <VStack spacing={8} align="center">
      {spinsLeft <= 0 && !mustSpin && (
        <HStack spacing={4}>
          <Button colorScheme="blue" onClick={() => handlePayment(1)}>Pay $10 to Play</Button>
          <Button colorScheme="blue" onClick={() => handlePayment(1)}>Pay $20 to Play</Button>
        </HStack>
      )}
      {(spinsLeft > 0 || mustSpin) && (
        <>
          <Text color="black" fontSize="xl">Spins Left: {spinsLeft}</Text>
          <AnimatePresence>
            {showWinAnimation && (
              <motion.div>
                <Text color="yellow.300" fontSize="6xl" fontWeight="bold">${lastWin}</Text>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <HStack spacing={4}>
            <motion.div whileHover={{ scale: 1.1 }} whileTap={{ simport React, { useEffect, useState } from "react";
import { 
  Box, 
  Button, 
  Text, 
        ...
        <>
          ...
          <HStack spacing={4}>
            <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
              <Button colorScheme="teal" onClick={handleSpinClick} isDisabled={mustSpin || spinsLeft === 0}>Spin</Button>
            </motion.div>
          </HStack>
        </>
      )}
